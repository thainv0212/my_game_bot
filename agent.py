...
from ddqn import advantage, advantage_per
...
class AgentWithNormalMemory():
    def __init__(self, gamma=0.9, replace=100, lr=0.001, epsilon=1.0, action_space_num=(56)):
        self.gamma = gamma
...
class AgentWithPERAndMultiRewards(AgentWithNormalMemory):
    ...
    def save_model(self):
        self.q_net_offensive.save('q_net_per_offensive')
        self.q_net_defensive.save('q_net_per_defensive')
        self.target_net_offensive.save('target_net_per_offensive')
        self.target_net_defensive.save('target_net_per_defensive')

    def load_model(self):
        tmp = load_model("q_net_per_offensive")
        self.q_net_offensive = tmp
        tmp = load_model('q_net_per_defensive')
        self.q_net_defensive = tmp
        tmp = load_model("target_net_per_offensive")
        self.target_net_offensive = tmp
        tmp = load_model('target_net_per_defensive')
    ...
    def train(self):
        ...
        q_target_offensive[batch_index, actions] = rewards_offensive + self.gamma * next_states_val_offensive[
    ...
    def act(self, state):
        ...
        if np.random.rand() <= self.epsilon or state is None:
            ...
            try:
                np_state = np.squeeze(state).transpose().flatten()
                np_state = np.expand_dims(np_state, axis=0)
                offensive_actions = advantage_per(self.q_net_offensive, np_state)
                defensive_actions = advantage_per(self.q_net_defensive, np_state)
                actions = offensive_actions + defensive_actions
                action = np.argmax(actions)
            except Exception as ex:
                action = np.random.choice([i for i in range(self.action_space_num)])
                raise ex
...
class AgentNormalMultiReward(AgentWithNormalMemory):
    def __init__(self, gamma=0.9, replace=100, lr=0.001, epsilon=1.0, action_space_num=(56)):
        ...
        self.target_net_defensive.compile(loss='mse', optimizer=opt)
        self.action_space_num = action_space_num
        self.memory_file = 'memory_normal_multi_per.pkl'
        try:
            self.load_model()
            self.load_memory()
        except:
    ...
    def train(self):
        ...
        q_target_offensive[batch_index, actions] = rewards_offensive + self.gamma * next_states_val_offensive[
            batch_index, max_action] * (1 - dones)
        loss_offensive = self.q_net_offensive.train_on_batch(states, q_target_offensive)
    ...
    def act(self, state):
        ...
        if np.random.rand() <= self.epsilon or state is None:
            ...
            try:
                np_state = np.squeeze(state).transpose().flatten()
                np_state = np.expand_dims(np_state, axis=0)
                offensive_actions = advantage(self.q_net_offensive, np_state)
                defensive_actions = advantage(self.q_net_defensive, np_state)
                actions = offensive_actions + defensive_actions
                action = np.argmax(actions)
            except Exception as ex:
                action = np.random.choice([i for i in range(self.action_space_num)])
